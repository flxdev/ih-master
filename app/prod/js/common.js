...
$(document).ready(function () {
    ...
    var inp = $('.form').find('.input__field');
                    ...
                    onSuccess: function($form){
                        ...
                        if(typeof yaCounter34955600 != 'undefined'){
                            ...
                            if($form.hasClass('register_form')){
                                yaCounter34955600.reachGoal('conf');
                                ga('send', 'event', 'confr', 'reg');
                            }
                        }
                        return false;
                    }
    ...
    (function(){
        ...
        var oldVal;
        $('.js-input-tel-multi').parent().on('keyup paste textInput input', '.js-input-tel-multi', function (e) {
            var val = $(this).val(),
                numbers = val.match(/(\d+)*/g).join('');

            if(numbers == 7 && oldVal != numbers) {
                $(this).mask("+7 (999) 999 99 99").val(val);
                oldVal = numbers;
            }
            else if(numbers == 375 && oldVal != numbers)
            {
                $(this).mask("+375 (99) 999 99 99").val(val);
                oldVal = numbers;
            }
            else if(e.keyCode == 8 && oldVal == numbers) {
                oldVal = null;
                $(this).unmask().val('+' + numbers);
            }
        });
        ...
        if ($(".js-input-tel").length) {
            $(".js-input-tel").mask("+375 (99) 999 99 99");
            $(".js__birthday").mask("99.99.9999");
        }
    })();
    ...
    function popups(){
        ...
        $(".popup__close, .return").on("click", function(e){
            if(!popupSelector.hasClass('is-visible')) return;
...
$(function() {
    ...
    $b_startTest.on('click', function() {
        ...
        $pu_startTest.fadeIn(400, function() {
            $pu_startTest.addClass('is-visible').find('.popup__form').addClass('is-active');
            // TODO
            // Нужно временно закрыть раздел тестирования
